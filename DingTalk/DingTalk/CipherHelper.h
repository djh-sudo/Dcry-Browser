...
public:
	...
	static std::string AesDecrypt(unsigned char *in, int size, unsigned char *key) {
		AES_KEY aesKey = { 0 };
		int status = AES_set_decrypt_key(key, 128, &aesKey);
		if (status < 0) {
			return "";
		}
		std::string out = "";
		int blockSize = size / AES_BLOCK_SIZE;
		unsigned char buffer[AES_BLOCK_SIZE] = { 0 };
		for (int i = 0; i < blockSize; ++i) {
			memset(buffer, 0, AES_BLOCK_SIZE);
			AES_decrypt(in + (i << 4), buffer, &aesKey);
			out += std::string((char *)buffer, AES_BLOCK_SIZE);
		}
		return out;
	}
